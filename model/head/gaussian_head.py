...
class GaussianHead(BaseTaskHead):
    def __init__(
        ...
        **kwargs,
        ...
        self.SupervisedOnBEV = SupervisedOnBEV
        if with_empty:
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.SupervisedOnBEV:
            occ_cam_mask = occ_cam_mask.max(dim=3).values
            bev_label = occ_label.max(dim=3).values
            sampled_label = bev_label.flatten(1)
            bev_xyz = occ_xyz[:, :, :, 0, :].clone()
            bev_xyz[..., 2] = 0
            sampled_xyz = bev_xyz.view(bs, -1, 3)
        else:
        ...
        for idx in apply_loss_layers:
            ...
            if self.use_localaggprob:
                if self.combine_geosem:
                    ...
                    geosem = semantics[0]
                if self.SupervisedOnBEV:
                    geosem = geosem.view(H * W, Z, -1).max(dim=1).values
                    prediction.append(geosem[None].transpose(1, 2))
                    bin_logits.append(semantics[1].view(H * W, Z).max(dim=1).values[None])
                    density.append(semantics[2].view(H * W, Z).max(dim=1).values[None])
                else:
                    prediction.append(geosem[None].transpose(1, 2))
                ...
                if self.SupervisedOnBEV:
                    sem = semantics.view(H * W, Z, -1).max(dim=1).values
                    prediction.append(sem[None].transpose(1, 2))
                else:
